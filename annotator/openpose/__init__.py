...
body_model_filename = "body_pose_model.pth"
hand_model_filename = "hand_pose_model.pth"
face_model_filename = "facenet.pth"
...
class OpenposeDetector:
    def __init__(self):
        ...
        if not os.path.exists(body_modelpath):
            hf_hub_download(repo_id=repo_id, filename=body_model_filename, local_dir = annotator_ckpts_path)

        if not os.path.exists(hand_modelpath):
            hf_hub_download(repo_id=repo_id, filename=hand_model_filename, local_dir = annotator_ckpts_path)

        if not os.path.exists(face_modelpath):
            hf_hub_download(repo_id=repo_id, filename=face_model_filename, local_dir = annotator_ckpts_path)
    ...
    def __call__(self, oriImg, hand_and_face=False, return_is_index=False):
        ...
        with torch.no_grad():
            ...
            if return_is_index:
                meta = dict(candidate=candidate.tolist(), subset=subset.tolist())
                return meta
            else:
